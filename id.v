 ...
 ex_aluop_i == `EXE_SWRS_OP) ? 1'b1 : 1'b0; 
  assign inst_o = inst_i;
				...
				case (op4)
					...
					`INST_JR:		begin
						case (op3)
							...
							3'b110: begin //JALR
								wreg_o <= `WriteEnable;		aluop_o <= `EXE_J_OP;
								alusel_o <= `EXE_RES_JUMP_BRANCH; reg1_read_o <= 1'b1;	reg2_read_o <= 1'b0;
								link_addr_o <= pc_i + 16'b1;
								branch_target_address_o <= reg1_data_i;
								branch_flag_o <= `Branch;
								next_inst_in_delayslot_o <= `InDelaySlot;		  	
								instvalid <= `InstValid;	
			...
			`EXE_SLLSRA: begin
				case(inst_i[1])
					1'b1: begin
						...
						imm[2:0] <= inst_i[4:2];		wd_o <= inst_i[10:8];
						instvalid <= `InstValid;
					end
					1'b0: begin
	...
	always @ (*) begin
		...
		if(pre_inst_is_load == 1'b1 /*&& ex_wd_i == reg1_addr_o 
						&& reg1_read_o == 1'b1 */) begin
		  stallreq_for_reg1_loadrelate <= `Stop;	
		 end
		 if(pre_inst_is_load == 1'b1 /*&& ex_wd_i == reg2_addr_o 
